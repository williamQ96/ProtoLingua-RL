...
if len(sys.argv) < 2:
    print("Usage: python train_hybrid_entropy.py <num_episodes>")
    sys.exit(1)

num_episodes = int(sys.argv[1])
progress_bar = tqdm(total=num_episodes, desc="Training", dynamic_ncols=True)
log_filename = f"logs/reward_log_hybrid_entropy_annealed_{num_episodes}.txt"
...
for episode in range(num_episodes):
    ...
    # Quadratic decay entropy weight
    entropy_weight = 0.01 * (1 - (episode / num_episodes) ** 2)
    if episode % 1000 == 0:
        debug_print(f"[DEBUG] Entropy weight: {entropy_weight:.4f}")
